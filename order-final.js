...
function updateRequiredSizes() {
  ...
  jerseySize.required = !JERSEY_SOLD_OUT && Number(jerseyQuantity.value) > 0;
  teeSize.disabled = Number(teeQuantity.value) === 0;
  jerseySize.disabled = JERSEY_SOLD_OUT || Number(jerseyQuantity.value) === 0;
...
function updateSummary() {
  ...
  teeSummaryValue.textContent = order.teeCount > 0
    ...
    : "0 ks";
  jerseySummaryValue.textContent = JERSEY_SOLD_OUT
    ? "VYPRODÁNO"
    : (order.jerseyCount > 0 ? `${order.jerseyCount} ks · ${jerseySize.value || "bez velikosti"}` : "0 ks");
...
if (requestedProduct === "tee") teeQuantity.value = "1";
if (JERSEY_SOLD_OUT) jerseyQuantity.value = "0";
...
form.addEventListener("submit", async (event) => {
  ...
  orderSubmit.textContent = "ODESÍLÁM…";
  ...
  try {
    ...
    if (!response.ok || data.success === false) throw new Error("Odeslání se nezdařilo");
    window.location.href = "dekujeme.html";
  } catch (error) {
    submitError.textContent = "Objednávku se nepodařilo odeslat. Zkus to prosím znovu nebo nás kontaktuj na Instagramu @voskytt.";
    orderSubmit.disabled = false;
    orderSubmit.textContent = "ODESLAT OBJEDNÁVKU";
    sending = false;
  }
});
